...
const SocialFeedPage = () => {
  ...
  const handleUploadInformation = (obj) => {
        if(obj?.type === 'tweetType'){
          setSelectedTweet({value: obj.value})
          return;
        }
        if(obj?.type === 'platform'){
          setSelectedPlatform({value: obj.value})
        }
    }
  return (
    <OperationLayout
    ...
    >
      ...
      <div className="gradient-add-bookmark-div">
                  <img src={details?.imageUrl || `${process.env.REACT_APP_STATIC_IMAGES_CDN}/webapp/curateit-logo.png`}
